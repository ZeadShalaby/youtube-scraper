    ...
    <div
        ...
        <div class="flex flex-row justify-between p-2">
            <div class="flex flex-row">
                ...
                <img class="w-10 h-10 rounded-full" src="{{ asset($item->userfollowing->media_one->media) }}"
                    alt="profile-follow" />

                <div class="flex flex-col ml-2">
                    <h1 class="text-gray-900 dark:text-white font-bold text-sm">

                        {{ $item->userfollowing->name }}

                    </h1>
                    <p class="text-gray-400 text-sm">@

                        {{ $item->userfollowing->username }}
            ...
            <div class="">
                <div class="flex items-center h-full text-gray-800 dark:text-white">
                    <form action="{{ route('follow.unfollow', ['followed_id' => $item->id]) }}" method="POST">
                        {{ csrf_field() }}
