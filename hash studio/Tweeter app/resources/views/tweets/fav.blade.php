        ...
        <div
                ...
                <a href="#" class="flex-shrink-0 group block">
                    <div class="flex items-top">
                        <div>
                            ...
                            @if (isset($tweet->tweet->user->media) && $tweet->tweet->user->media != null)
                                @foreach ($tweet->tweet->user->media as $media)
                                    <img class="inline-block h-9 w-9 rounded-full" src="{{ $media->media }}"
                                        alt="{{ $media->media }}" />
                                @endforeach
                            @endif
                        </div>
                        ...
                        <div class="ml-3">
                            <p class="flex items-center text-base leading-6 font-medium text-gray-800 dark:text-white">
                                @if (isset($tweet->tweet->user) && $tweet->tweet->user != null)
                                ...
                                <span
                                    ...
                                    @if (isset($tweet->tweet->user) && $tweet->tweet->user != null)
                                        ...
                                        . {{ $tweet->tweet->creation_date_formatted }}
                                    @endif
                                </span>

                                <!-- Dropdown Menu -->
                                @component('components.dropdown-menu', ['tweet' => $tweet])
                                @endcomponent
                                <!-- / Dropdown Menu -->
                            </p>
                        </div>
            ...
            <div class="pl-16">
                ...
                <div class="flex my-3 mr-2 rounded-2xl border border-gray-600">
                    @if (isset($tweet->tweet_media) && $tweet->tweet_media != null)
                        ...
                        @endforeach
                    @else
                        <img class="rounded-2xl" src="{{ asset('images/img/404.png') }}" alt="image media" />
                    @endif
                ...
                <div class="flex">
                    <div class="w-full">
                        <div class="flex items-center">
                            @if (isset($tweet->tweet->id) && $tweet->tweet->id != null)
                                <!--- favourite --->
                                <div
                                    ...
                                    <form action="{{ route('fav-tweet.store', ['tweet_id' => $tweet->id]) }}"
                                        method="POST">
                                        {{ csrf_field() }}
                                        ...
                                        <button style="background: none; border: none" id = 'toggleButton'>
                                            ...
                                            <a href="#">
                                                <svg fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6"
                                                    style="color: {{ $tweet->tweet->isFavoritedBy(auth()->user()) ? '#ff0000' : '#000000' }}">
                                ...
                                <div
                                    ...
                                    <form action="{{ route('likes.store', ['tweet_id' => $tweet->id]) }}" method="POST">
                                        ...
                                        <button style="background: none; border: none">
                                            <a href="#">
                                                ...
                                                <svg viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 mr-2"
                                                    style="color: {{ $tweet->tweet->isLikedBy(auth()->user()) ? '#ff0000' : '#000000' }}">
                                                    >
                                                    <g>
                                                        <path
                                    ...
                                    {{ $tweet->tweet->getLikesCount() }}
                                </div>
                                <!--- explore --->
                                <div
                                    class="flex-1 flex items-center text-gray-800 dark:text-white text-xs text-gray-400 hover:text-blue-400 dark:hover:text-blue-400 transition duration-350 ease-in-out">
                                    <form action="{{ route('exploreindex', $tweet->tweet->id) }}" method="POST">
                                        {{ csrf_field() }}
                                        <button style="background: none; border: none">
                                            <a href="#">
                                    ...
                                    {{ $tweet->tweet->explore !== null && $tweet->tweet->explore != 0 ? $tweet->tweet->explore : 0 }}
                                </div>
                                <!--- report --->
                                <div
                                    class="flex-1 flex items-center text-gray-800 dark:text-white text-xs text-gray-400 hover:text-blue-400 dark:hover:text-blue-400 transition duration-350 ease-in-out">
                                ...
                                <div
                                    ...
                                    {{ $tweet->tweet->view !== null && $tweet->tweet->view != 0 ? $tweet->tweet->view : 0 }}
