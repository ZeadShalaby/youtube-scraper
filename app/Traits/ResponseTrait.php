...
{  
    ...
    protected function CheckField($request)
    {
        ...
        $field = filter_var($request->input('field'),FILTER_VALIDATE_EMAIL) ? 'gmail' : 'username';
        $credentials = array(
   ...
   // todo return 
    ...
    // todo return check error for tasks 
    public function checkTasks($tasks)
    {
        if(!$tasks->id){return $this->returnError('P404','Error Some Thing Wrong .');}
        if($tasks->user_id != auth()->user()->id){return $this->returnError('U303','Error Some Thing Wrong .');} 
    }
